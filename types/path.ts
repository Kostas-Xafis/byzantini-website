...
type GetLiteral<Literal> = Literal extends `"${infer Str}"` ? Str : Literal extends `${infer Num}` ? ToDigit<Num> : never;

type ExtractLiteral<Param> = Param extends `${infer Literal} | ${infer Next}`
...
type ExtractArgumentParts<Path extends string> = Path extends `${infer _}[${infer Name}:${infer Type}]${infer Next}`
	? `[${Name}:${Type}]` | ExtractArgumentParts<Next>
	: never;
export type ExpectedArguments<URL extends string> = {
	[K in ExtractArgumentParts<URL> as ExtractName<K>]: ExtractType<K> extends keyof StringToType
