...
export type EndpointRoute<URL extends string, Req = any, Res = string> =
	IsAny<URL> extends false
		? {
				authentication: boolean;
				method: HTTPMethods;
				path: URL;
...
export type APIEndpointsBuilder<Mount extends string, Routes extends Record<string, AnyEndpoint>> = {
...
export type APIArguments<Mount extends string, Routes extends Record<string, AnyEndpoint>> = {
	[K in keyof Routes as ConcatStrings<Mount, K & string, ".">]: RemoveNullishFields<{
		RequestObject: Parameters<RemovePartial<Routes[K], "func">["func"]>[0]["ctx"]["request"] extends { json: () => Promise<infer T> }
...
export type APIResponse<Mount extends string, Routes extends Record<string, AnyEndpoint>> = {
	[K in keyof Routes as ConcatStrings<Mount, K & string, ".">]: RemovePartial<Routes[K], "func">["func"] extends (...args: any) => Promise<infer T>
		? T extends DefaultEndpointResponse
			? ExtractData<T>
			: string
		: string;
};
