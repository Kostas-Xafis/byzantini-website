...
export const onDownloadExcel = function (store: Partial<APIResponse>, selectedItems: number[]) {
	return createMemo((): Action | EmptyAction => {
		...
		if (!teachers || !classes || !instrumentsByTeacher || !instruments || selectedItems.length <= 0) return excelModal;
		const submit = async function () {
			...
			const wsStudentsBook = xlsx.utils.aoa_to_sheet(
				[["Ονοματεπώνυμο", "Ιδιότητα", "Email", "Τηλέφωνο", "ΑΜΚΑ", "Αριθμός Έγκρισης"]].concat(
					byzTeachers.map((t) => {
						const ao = classes.find((c) => c.teacher_id === t.id && c.class_id === 0)?.registration_number;
						return [
