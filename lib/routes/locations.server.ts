...
serverRoutes.post.func = async req => {
	return await execTryCatch(async () => {
		...
		const { insertId } = await executeQuery(
			`INSERT INTO locations (name, address, areacode, municipality, email, telephones, priority, map, link) VALUES (${questionMarks(args.length)})`,
			args
		);
		return { insertId };
	});
...
serverRoutes.update.func = async req => {
	return await execTryCatch(async () => {
		const body = await req.json();
		const args = Object.values(body);
		await executeQuery(`UPDATE locations SET name=?, address=?, areacode=?, municipality=?, email=?, telephones=?, priority=?, map=?, link=? WHERE id=?`, [
			...args.slice(1),
			body.id
		]);
