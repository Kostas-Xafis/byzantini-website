...
const deleteOldFiles = (ctx: APIContext) => execTryCatch(async () => {
	const teachers = await executeQuery<{ fullname: string, cv?: string; picture?: string; }>("SELECT fullname, picture, cv FROM teachers");
	for (const { fullname, cv, picture } of teachers) {
		if (cv) await Bucket.delete(ctx, cv);
		if (picture) await Bucket.delete(ctx, picture);
		const cvFileType = cv?.split(".")[1];
		const pictureFileType = picture?.split(".")[1];

		const newCVName = cv ? cvPrefix + fullname + "." + cvFileType : "";
		const newPictureName = picture ? picturePrefix + fullname + "." + pictureFileType : "";
		if (!cv && !picture) continue;
		await executeQuery("UPDATE teachers SET cv = ?, picture = ? WHERE fullname = ?", [newCVName, newPictureName, fullname]);
	}
